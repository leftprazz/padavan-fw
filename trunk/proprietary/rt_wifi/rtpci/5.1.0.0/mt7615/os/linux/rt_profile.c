...
UINT get_dev_eeprom_offset(VOID *pvAd)
{
	...
	return (l1profile[get_dev_config_idx(pAd)].ee_info.offset & 0xFFFF);
}
	...
	IN UCHAR OpMode)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		if (RTMP_MATPktRxNeedConvert(pAd, GET_OS_PKT_NETDEV(pRxPkt))) {
#if defined(CONFIG_WIFI_PKT_FWD) || defined(CONFIG_WIFI_PKT_FWD_MODULE)
...
#ifdef DYNAMIC_VLAN_SUPPORT
	{
		...
		if (ra_sw_nat_hook_rx != NULL) {
			...
			RTPKT_TO_OSPKT(pRxPkt)->protocol = eth_type_trans(RTPKT_TO_OSPKT(pRxPkt), GET_OS_PKT_NETDEV(pRxPkt));
			FOE_MAGIC_TAG(RTPKT_TO_OSPKT(pRxPkt)) = FOE_MAGIC_EXTIF;

			if (ra_sw_nat_hook_rx(RTPKT_TO_OSPKT(pRxPkt))) {
				FOE_MAGIC_TAG(RTPKT_TO_OSPKT(pRxPkt)) = 0;
				netif_rx(RTPKT_TO_OSPKT(pRxPkt));
			}
	...
	if (ra_sw_nat_hook_set_magic)
		ra_sw_nat_hook_set_magic(pRxPkt, FOE_MAGIC_EXTIF);
